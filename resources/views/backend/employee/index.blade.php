...
@section('content')
    <div class="p-6">
        ...
        <div class="flex items-center justify-end mb-2">
            <label class="flex items-center space-x-2 text-red-800 font-medium">
                <input type="checkbox" id="show-deleted" name="show_deleted" class="form-checkbox h-5 w-5 text-red-600"
                    {{ $showDeleted ? 'checked' : '' }}>
                <span class="text-xs">Show Deleted Employee</span>
            </label>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-semibold text-gray-700">Designation</th>
                        ...
                        <tr class="bg-white hover:bg-gray-50 transition duration-150 ease-in-out">
                            ...
                            <td class="px-4 py-2">
                                <img src="{{ asset('storage/' . $employee->image_path) }}"
                                    alt="image of {{$employee->name}}" class="object-cover rounded" style="width: 64px; height: 64px;">
                            </td>
                            <td class="px-4 py-2">{{ $employee->name }}</td>
                            {{-- <td class="px-4 py-2">{{ $employee->title_en }}</td> --}}
                            <td class="px-4 py-2">{{ $employee->post->title ?? 'N/A' }}</td>
                            <td class="px-4 py-2">{{ $employee->department->title ?? 'N/A' }}</td>
        ...
        <div class="mt-4">
            {{ $employees->links() }} {{-- Pagination links if using paginate() --}}
        </div>
    </div>
