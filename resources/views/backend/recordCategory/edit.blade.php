...
@section('title', 'Edit Record Category')

@section('content')
    <div class="container mx-auto py-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Edit Record Category</h1>

        <form action="{{ route('record-categories.update', $recordCategory->id) }}" method="POST"
            ...
            <div class="mb-6 w-full">
                ...
                <input type="text" name="title" id="title" required
                    value="{{ old('title', $recordCategory->title) }}"
            ...
            <div class="mb-6 w-full">
                ...
                <input type="text" name="title_en" id="title_en"
                    value="{{ old('title_en', $recordCategory->title_en) }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"
            ...
            <div class="mb-6 w-full">
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $recordCategory->slug) }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"
                    placeholder="Auto-generated slug" x-data="{ slug: '{{ old('slug', $recordCategory->slug) }}' }" x-init="generateSlug()"
            ...
            <div class="mb-6 w-full">
                ...
                <select name="record_type_id" id="record_type_id" required
                    ...
                    @foreach ($recordTypes as $recordType)
                        <option value="{{ $recordType->id }}"
                            {{ old('record_type_id', $recordCategory->record_type_id) == $recordType->id ? 'selected' : '' }}>
                            {{ $recordType->title }} ({{ $recordType->title_en }})
                        </option>
                    @endforeach
                </select>
